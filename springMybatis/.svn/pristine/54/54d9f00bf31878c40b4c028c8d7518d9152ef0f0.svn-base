	window.onload = function() {
			...
			function draw() {
				...
				for ( var i = 0; i < particles.length; i++) {
					...
					var gradient = ctx.createRadialGradient(p.location.x, p.location.y, 0,
							p.location.x, p.location.y, p.radius);
					gradient.addColorStop(0, "rgba(" + p.r + ", " + p.g + ", " + p.b + ", "
							+ p.opacity + ")");
					gradient.addColorStop(0.5, "rgba(" + p.r + ", " + p.g + ", " + p.b + ", "
							+ p.opacity + ")");
			...
			updateSize();
			function updateSize(){
				$("#canvas-keleyi-com").width($(window).width());
				$("#canvas-keleyi-com").height($(window).height());
			}
		}
