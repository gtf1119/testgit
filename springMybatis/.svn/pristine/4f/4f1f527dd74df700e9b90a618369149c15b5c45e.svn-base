...
var buildOrder = {
	...
	build : function(args){
		...
		$(pageTr).html(html);
		pageObj.condition = args.data.condition;
	...
	eventBind : function(args){
		...
		$(window).scroll(function(){
			var waitScroll = $(".orderContainer:last").offset().top - $(".orderContainer:last").height();
			...
			if(navigator.userAgent.indexOf("iPhone") > -1){
				...
				if($("body").scrollTop() > waitScroll){
					...
					$.post("queryOrder.action",pageObj,function(json){
						pageObj.condition = json.condition;
